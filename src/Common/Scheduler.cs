...
namespace Common
{
    public class Scheduler
    {
        ...
        /// 如果任务调度器未连接，则连接
        /// </summary>
        private static TaskSchedulerClass TryConnectScheduler()
        {
        ...
        {
            outTask = null;
            TaskSchedulerClass scheduler = TryConnectScheduler();
        ...
        public static IRegisteredTask InstallAutoRunClient(string taskName, string exeFullPath, string taskDesc, string args = null)
        {
            TaskSchedulerClass scheduler = TryConnectScheduler();
            ...
            if (!string.IsNullOrEmpty(args))//启动参数
            {
        ...
        public static void UninstallAutoRunClient(string taskName)
        {
            TaskSchedulerClass scheduler = TryConnectScheduler();

            if (!TryGetTask(taskName, out var outtask))
            {
                throw new InvalidOperationException("计划任务不存在");
            }
